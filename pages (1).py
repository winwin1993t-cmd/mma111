...
.vmess-box{background:linear-gradient(135deg,var(--bg3) 0%,var(--bg2) 100%);border:1px solid var(--card-b);border-radius:18px;padding:20px 22px;margin-bottom:18px;box-shadow:var(--shadow);position:relative;overflow:hidden;transition:background .3s}
.vmess-box::before{content:'';position:absolute;top:-50px;left:-50px;width:180px;height:180px;background:radial-gradient(circle,var(--accent-d),transparent 70%);pointer-events:none}
...
<section class="pg on" id="pg-overview">
  ...
  <div class="metrics">
    <div class="metric"><div class="m-icon"><i class="ti ti-plug-connected"></i></div><div class="m-label">اتصالات فعال</div><div class="m-val" id="m-conns">—</div><div class="m-sub"><span class="dot dg pulse"></span> VMess WebSocket زنده</div></div>
  ...
  <div class="vmess-box">
    <div class="vl-header">
      <div class="vl-title"><i class="ti ti-link"></i> VMess لینک پیش‌فرض (بدون محدودیت)</div>
      <span class="badge bg-blue"><span class="dot db"></span> TLS 443 · WS</span>
    </div>
    <div class="vl-code" id="vmess-main">در حال دریافت...</div>
    <div class="vl-actions">
      <button class="btn btn-p" onclick="cpText('vmess-main')"><i class="ti ti-copy"></i> کپی</button>
      <button class="btn btn-g" onclick="qrFor('vmess-main')"><i class="ti ti-qrcode"></i> QR</button>
  ...
  <div class="g2">
    <div class="card">
      ...
      <div class="sr"><span class="sr-k"><i class="ti ti-circle-check"></i> VMess / WS Tunnel</span><span class="sr-v" style="color:var(--green-t)">● فعال</span></div>
...
<option value="vmess-ws">VMess / WebSocket</option>
        ...
        <div class="proto-cards">
          <div class="proto-card active" data-val="vmess-ws" onclick="selectProto('vmess-ws',this)">
  ...
  return `<div class="cfg-card ${cardCls}">
    <div class="cfg-row">
      ...
      <div class="cfg-actions">
        ...
        <button class="btn btn-sm btn-g btn-icon" onclick="navigator.clipboard.writeText('${esc(l.vmess_link)}').then(()=>toast('لینک کپی شد','ok'))" title="کپی لینک"><i class="ti ti-copy"></i></button>
        ...
        <button class="btn btn-sm btn-g btn-icon" onclick="showQR('${esc(l.vmess_link)}')" title="QR"><i class="ti ti-qrcode"></i></button>
...
async function fetchDefaultVmess(){
  try{const r=await authF('/api/links'),d=await r.json();const links=d.links||[];const def=links.find(l=>l.limit_bytes===0&&l.active&&!l.expired)||links.find(l=>l.active&&!l.expired)||links[0];document.getElementById('vmess-main').textContent=def?def.vmess_link:'هنوز کانفیگی وجود ندارد';}catch(e){}
}
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  fetchStats();fetchDefaultVmess();loadLinks();loadSubs();
...
.cfg-vmess-wrap{{display:grid;grid-template-rows:0fr;transition:grid-template-rows .25s ease}}
.cfg-vmess-wrap.open{{grid-template-rows:1fr}}
.cfg-vmess-inner{{overflow:hidden}}
.cfg-vmess{{background:rgba(0,0,0,.22);border:1px solid var(--card-b);border-radius:10px;padding:11px 13px;font-size:9.8px;font-family:ui-monospace,monospace;color:var(--accent2);word-break:break-all;line-height:1.7;margin-top:9px;max-height:90px;overflow-y:auto}}
[data-theme="light"] .cfg-vmess{{background:rgba(46,99,214,.05)}}
...
function protoChip(p){{
  ...
  return '<span class="proto-chip pc-ws">VMess · WS</span>';
}}
        ...
        return `
          <div class="cfg-card${{l.active ? '' : ' inactive'}}">
            ...
            <div class="cfg-bottom">
              ...
              <div class="cfg-vmess-wrap" id="vw-${{i}}">
                <div class="cfg-vmess-inner">
                  <div class="cfg-vmess">${{esc(l.vmess_link)}}</div>
                </div>
              </div>
              <div class="cfg-actions">
                <button class="btn btn-p"
                  onclick="navigator.clipboard.writeText(window._x4gLinks[${{i}}].vmess).then(()=>toast('لینک کپی شد ✓','ok'))">
                  <i class="ti ti-copy"></i> کپی لینک
                </button>
                <button class="btn btn-g"
                  onclick="showQR(window._x4gLinks[${{i}}].label, window._x4gLinks[${{i}}].vmess)">
...
function copyAllConfigs(){{
  ...
  const text=links.map(l=>l.vmess).join('\\n');
